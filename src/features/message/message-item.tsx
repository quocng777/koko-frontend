  ...
  return (
    <div className='w-full grid'>
        ...
        className={`${metaData.isMe ? 'justify-self-end' : ''}`}>
                ...
                <div className={`${metaData.isMe ? 'grid justify-items-end' : ''}`}>
                    {metaData.showUserInfo && 
                        <div className="text-xs font-medium flex gap-3 my-1">
                            ...
                            <p className="text-slate-500">{metaData.formattedCreatedAt}</p>
                        </div>}
            ...
            {
                isLatestMessage && metaData.isMe && <MessageSendingStatus message={message}/>
            }
            {
                message.type !== MessageType.TEXT && !isLatestMessage && metaData.isMe && <MessageSendingStatus message={message} unShowIfSent={true}/>
            }
        </div>
    </div>
  );
...
const MessageSendingStatus = ({message, unShowIfSent = false}: {message: Message, unShowIfSent?: boolean}) => {
    ...
    if((messageStatus === 'Seen' || messageStatus === 'Sent') && unShowIfSent)
        return undefined

    return <div className='flex justify-end'>
        <span className={`text-xs font-medium px-3 ${message.hasError ? 'text-red-500' : ''}`}>{messageStatus}</span>
    </div>
}
